...
class DeepKnn(torch.nn.Module):
    def __init__(self, cfg, dataset) -> None:
        # TODO add support for loading specific embedders
        self.embedders = {modality: torch.nn.Identity() for modality in cfg.modalities}
        self.embeddings = self.update_embeddings(torch.utils.data.DataLoader(dataset, shuffle=False))
        modality_weights = [1 for _ in cfg.modalities]
        self.knn_retriever = KNN_Model(
    ...
    def fit_retriever(self, df):
        self.knn_retriever.fit(df)

    def embedder(self, batch):
        embedded_batch = {}
        for modality, data in batch.items():
            embedded_batch[modality] = self.embedders[modality](data)
        return embedded_batch

    def update_embeddings(self, dataloader):
        with torch.no_grad():
            embeddings = []
            for batch in dataloader:
                embeddings.append(self(batch).cpu().to_numpy())

    def embed_neighbors(self, neighbors):
        # TODO
        pass

    def compute_distance(self, query_embeddings, neighbor_embeddings):
        # TODO
        pass

    def compute_weighted_sum(self, distances, neighbor_labels):
        # TODO
        pass


def forward(self, batch):
    # compute the neighbor scores
    neighbors = []
    neighbor_labels = []
    for each in batch:
        retrieved_neighbors, labels = self.knn_retriever.predict(each)
        neighbors.append(retrieved_neighbors)
        neighbor_labels.append(labels)
    # embed neighbors
    neighbor_embeddings = self.embed_neighbors(neighbors)
    # embed query
    query_embeddings = self.embedder(batch)
    # compute Inner Produce distances
    distances = self.compute_distance(query_embeddings, neighbor_embeddings)
    # TODO weighted sum of neighbor labels
    predictions = self.compute_weighted_sum(distances, neighbor_labels)
    return predictions
